...
TYPE_RULES: list[tuple[str, str]] = [
    ("부가세", r"부가가치세|세금계산서|매입세액|과세표준|간이과세|영세율|면세"),
    ("원천징수", r"원천징수|연말정산|일용직|근로소득|퇴직소득|사업소득"),
    ("소득세", r"소득세법|종합소득|기타소득|이자소득|배당소득|소득세"),
    ("감가상각", r"감가상각"),
    ("유가증권", r"단기매매증권|매도가능증권|만기보유증권|유가증권|자기주식|단기차익|시세차익"),
    # 매출원가·매출총이익 계산은 재고 흐름 문제 — 광범위한 '원가' 패턴보다 먼저
    ("재고자산", r"재고자산|선입선출|후입선출|총평균|이동평균|소모품|매출원가|매출총이익|상품재고|상품의 취득"),
    ("대손", r"대손"),
    ...
    ("채권채무", r"외상매출금|외상매입금|미수금|미지급금|선수금|선급금|대여금|매출채권|매입채무"),
    ("이자손익", r"미수수익|이자수익|이자비용|미지급비용|선급비용|선수수익"),
    ("현금예금", r"현금및현금성|현금성자산|당좌예금|정기예금|현금과부족|당좌차월|통화대용증권|당좌수표"),
    ("유형자산", r"유형자산|자본적 지출|취득원가|기계장치|차량운반구|건설중인자산|비품"),
    ("무형자산", r"무형자산|개발비|영업권"),
    ("자본", r"자본금|자본잉여금|이익잉여금|주식발행|증자|배당|자본조정|기타포괄|감자"),
    ("부채", r"유동부채|비유동부채|사채|충당부채"),
    ("재무상태표", r"재무상태표|기말자산|기말부채|기말자본|기초자본|유동자산|비유동자산|당좌자산|자산총계|부채총계|재무상태|유동성"),
    ("손익계산서", r"손익계산서|영업이익|당기순이익|영업외수익|영업외비용|판매비와\s?관리비|경영성과|세금과공과|복리후생비|여비교통비|기업업무추진비|접대비"),
    ("수익비용", r"수익의 인식|수익인식|발생주의|비용의 인식|수익비용|인식\s?시점|수익 인식"),
    ("원가", r"원가|노무비|제조간접비|가공비|공손|제조지시서|부문별|보조부문|당기총제조|당기제품제조"),
    ("회계원칙", r"회계의 순환|회계정보|재무제표|회계처리의 기본|회계상|회계변경|회계정책|회계추정|오류수정|시산표|거래요소|결합관계|계정의 기록|계정과목|복식부기|분개장|총계정원장|전표|결산\s?절차|재무회계|회계등식|거래의 종류"),
]
...
def parse_hwp_answer(path: Path) -> tuple[list[dict], list[dict]]:
    ...
    lines = [
        ln
        for ln in lines
        if not re.fullmatch(r"-?\s*\d*\s*/\s*\d+\s*-?|<\d+>|\[제\s*\d+\s*회[^\]]*\]", ln)
        and not re.match(
    ...
    for num, it in enumerate(items[:15], start=1):
        try:
            ...
            # 보기 마커 2개가 나란히("①  ②") = T계정 그림형 문제의 텍스트 붕괴 흔적
            if re.search(rf"[{GLYPHS}]\s*[{GLYPHS}]", stem):
                raise ValueError("T계정 그림형(마커 인접) — 텍스트 복구 불가")
