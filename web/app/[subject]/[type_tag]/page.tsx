...
/** 접힌 문제 목록 (stem은 HTML에 있어 색인 유지) */
function QuestionList({ list }: { list: Question[] }) {
  return (
    <>
      {list.map((q, i) => (
        <details
...
export default async function TypeTagPage({ params }: { params: Params }) {
  ...
  const all = await getBySubjectTag(s, t);
  ...
  return (
    <div className="space-y-5">
      <header className="card rise space-y-3 p-6">
        ...
        <p className="text-[14px] text-sub">
          이론 {theory.length}문항 · 보기를 누르면 바로 채점돼요
        </p>
